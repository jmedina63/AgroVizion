...
class TaskController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        ### search
        if ($request['query']) {
            $Task = Task::search($request['query'], null, false)->with('User')->with('Category')->paginate($per_page);
            return $Task;
        }
        return 'not found';
    }
        ...
        if (Auth::user()->can('add_task')) {
            if ($request) {
               ...
               // date_default_timezone_set('Europe/London');
                ...
                $request['start_date']=$datetime->format('Y-m-d');
                $datetime = new \DateTime($request['end_date']);
                $request['end_date']=$datetime->format('Y-m-d');
