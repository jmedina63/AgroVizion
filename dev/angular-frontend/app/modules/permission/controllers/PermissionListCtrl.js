...
app.controller('PermissionListCtrl',function($scope,PermissionService,resolvedItems,$translatePartialLoader,trans){
    ...
    $scope.pageChanged = function(per_page) {
        ...
        // console.log('perpage'+per_page.id);
        // console.log('query:'+$scope.query);
        PermissionService.search($scope.query,$scope.pagination.current_page,per_page.id).then(function(data){
            $scope.permissions = data;
            $scope.pagination = $scope.permissions.metadata;
            $scope.maxSize = 5;
        });
    ...
    $scope.search = function(per_page) {
        PermissionService.search($scope.query,1,per_page.id).then(function(data){
            $scope.permissions = data;
            $scope.pagination = $scope.permissions.metadata;
            $scope.maxSize = 5;
        });
    ...
    //    swal("Error", "Your records doesn't deleted :(", "error");
    //    PermissionService.list().then(function(data){
    //        $scope.permissions =data;
    //        $scope.selection=[];
    //    });
