...
angular.module('ng-laravel').service('UserService', function($rootScope, Restangular,CacheFactory) {
     ...
     * Search in users
     */
    this.search = function(query,pageNumber,per_page) {
        // GET /api/user/search?query=test&per_page=10
       return _userService.customGETLIST("search",{query:query, page:pageNumber,per_page:per_page});
    };
    ...
    this.downloadExport = function(recordType,selection,export_type){
        _userService.withHttpConfig({responseType: 'blob'}).customGET('export/file',{record_type:recordType,export_type:export_type,'selection[]':selection}).then(function(response) {
            ...
            anchor.href = url;
            anchor.click();
            $rootScope.$broadcast('user.export.download');
        },function (error) {
            $rootScope.$broadcast('user.export.download.fail');
        });
    };
    ...
    this.editProfile = function (user) {
        return _userService.customPUT({}, user.id + '/profile',user,{}).then(function (data) {
            $rootScope.$broadcast('user.updateProfile');
        },function (error) {
            $rootScope.$broadcast('user.validationError',error.data.error);
        });
    }
