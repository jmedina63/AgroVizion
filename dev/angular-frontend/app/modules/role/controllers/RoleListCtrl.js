...
app.controller('RoleListCtrl',function($scope,RoleService,resolvedItems,$translatePartialLoader,$rootScope,trans){
    ...
    $scope.pageChanged = function(per_page) {
        RoleService.search($scope.query,$scope.pagination.current_page,per_page.id).then(function(data){
            $scope.roles = data;
            $scope.pagination = $scope.roles.metadata;
            $scope.maxSize = 5;
        });
    ...
    $scope.search = function(per_page) {
        RoleService.search($scope.query,1,per_page.id).then(function(data){
            $scope.roles = data;
            $scope.pagination = $scope.roles.metadata;
            $scope.maxSize = 5;
        });
