...
app.controller('ForgetPassCtrl',function($scope,$auth,hotkeys,$state,Restangular,$stateParams){
    /*
    ...
    $scope.forget = function(resetemail){
        ...
        Restangular.all('password').customPOST('Email','email',{email:resetemail}).then(function(data) {
            ...
            swal({ title: "Error", text: response.data.email, type: "error"});
            $scope.send_text = "Send";
            $scope.isDisabled = false;
        });
    };
    ...
    $scope.reset = function(user){
        ...
        Restangular.one('password').customPOST('user','reset',user).then(function(data) {
            ...
            if(response.data.error){
                ...
                swal({ title: "Error", text: response.data.error, type: "error"});
            }
                ...
                swal({ title: "Error Validation", html: tmp, type: "error"});
            }
            $scope.send_text = "Send";
            $scope.isDisabled = false;
        })
